...
// MARK: - Interactive mouse-tracking view (D30)

/// Tracks the mouse and draws a crosshair + accent circle at the cursor position.
/// Also draws a filled rectangle from the last mouseDown position to the cursor.
final class MouseTrackerView: VNView {
    private var cursorPos: VNPoint? = nil
    private var dragStart: VNPoint? = nil
    private var dragEnd:   VNPoint? = nil

    override func draw(_ context: VNGraphicsContext) {
        // Subtle dark background
        context.fill(VNPath.rect(bounds),
                     color: VNColor(red: 0.05, green: 0.05, blue: 0.08, alpha: 1))
        ...
        if let s = dragStart, let e = dragEnd ?? cursorPos {
            ...
            context.fill(VNPath.roundedRect(r, cornerRadius: 4),
                         color: VNColor(red: 0.3, green: 0.6, blue: 1.0, alpha: 0.25))
            context.stroke(VNPath.roundedRect(r, cornerRadius: 4),
                           color: VNColor(red: 0.4, green: 0.7, blue: 1.0, alpha: 0.9),
                           lineWidth: 1.5)
        }
